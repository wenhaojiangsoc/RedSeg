# Does the redlining effect on RACIAL activity-space segregation vary by INCOME? (intersectionality via
# interaction/heterogeneity.) Boundary DiD (lgs:real) estimated within income terciles, two moderators:
#   (a) neighborhood current median income (median_hinc) -- descriptive; NOTE current income is partly
#       post-treatment (redlining depressed it), so read as association, not clean moderation;
#   (b) MSA median income (msa_inc) -- external to the POI, cleaner.
# Same entropy-balanced design/weights/clustering as the main estimator.
...
poi<-places_usa_2019%>%left_join(readRDS("data/poi_seg_fraccorr_as_rw.rds")%>%transmute(safegraph_place_id,seg_int_rw),by="safegraph_place_id")%>%
  mutate(seg_race=seg_int_rw*5/8,lvis=log(raw_visit_counts+1),tract11=substr(poi_cbg,1,11),
         nbhd_inc=median_hinc, msa_income=msa_inc)%>%
  filter(!is.na(seg_race),!is.na(lvis))%>%select(tract11,sub_category,seg_race,lvis,nbhd_inc,msa_income)
...
est<-function(sub){if(n_distinct(sub$boundary_id)<10) return(c(b=NA,s=NA)); m<-feols(seg_race~lgs+lgs:real|boundary_id+sub_category,sub,weights=~w,cluster=~boundary_id);c(b=unname(coef(m)["lgs:real"]),s=unname(se(m)["lgs:real"]))}
tercile<-function(x){q<-quantile(x,c(1/3,2/3),na.rm=TRUE);cut(x,c(-Inf,q,Inf),labels=c("Low","Mid","High"))}
run<-function(seed){d<-mkdat(seed)
  d$nbhd_t<-tercile(d$nbhd_inc); d$msa_t<-tercile(d$msa_income)
  # formal interaction: treatment x above-median income (each moderator)
  d$hi_n<-as.integer(d$nbhd_inc>median(d$nbhd_inc,na.rm=T)); d$hi_m<-as.integer(d$msa_income>median(d$msa_income,na.rm=T))
  gi<-function(v){dd<-d[!is.na(d[[v]]),]; dd$hi<-dd[[v]]; m<-feols(seg_race~lgs+lgs:real+lgs:real:hi+lgs:hi+hi|boundary_id+sub_category,dd,weights=~w,cluster=~boundary_id); c(b=unname(coef(m)["lgs:real:hi"]),s=unname(se(m)["lgs:real:hi"]))}
  bind_rows(
    tibble(dim="nbhd",grp=c("Low","Mid","High"),as_tibble(t(sapply(c("Low","Mid","High"),function(g)est(d[which(d$nbhd_t==g),]))))),
    tibble(dim="msa",grp=c("Low","Mid","High"),as_tibble(t(sapply(c("Low","Mid","High"),function(g)est(d[which(d$msa_t==g),]))))),
    tibble(dim="interaction",grp=c("nbhd_hiXtreat","msa_hiXtreat"),as_tibble(rbind(gi("hi_n"),gi("hi_m"))))
  )%>%mutate(seed=seed)}
R<-map_dfr(c(1,3,7,13,21),run); s<-R%>%group_by(dim,grp)%>%summarise(b=mean(b,na.rm=T),se=mean(s,na.rm=T),.groups="drop")%>%mutate(t=b/se,p=2*pnorm(-abs(t)))
cat("=== Racial boundary effect by INCOME tercile, and treatment x high-income interaction ===\n")
s%>%mutate(across(c(b,se),~round(.x,5)),t=round(t,2),p=round(p,4))%>%as.data.frame()%>%print(row.names=FALSE)
cat("\n(nbhd income is partly POST-treatment -> read as association; msa income is external.\n interaction rows: coefficient on treatment x above-median-income, i.e. how much larger/smaller the effect is.)\n")
saveRDS(R,"results/holc_income_interaction_FRACCORR.rds")
