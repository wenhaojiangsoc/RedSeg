...
d <- places_usa_2019 %>% left_join(rl, by="safegraph_place_id") %>% left_join(FRC, by="safegraph_place_id") %>% left_join(DIVF, by="safegraph_place_id") %>%
...
oster("seg_div_f")               # diversity, FRACTIONAL attribution
oster("race_seg_unadj_mean")     # diversity, majority (reference)
