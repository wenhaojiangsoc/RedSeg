...
catgrp12<-function(tc){tc<-as.character(tc);case_when(
  ...
  TRUE~"Excluded")}
S<-as_tibble(readRDS("results/poi_nonlocal_seg_FRACCORR.rds"))     # from-scratch measures + total visits t
...
rw<-readRDS("data/poi_seg_fraccorr_as_rw.rds")%>%transmute(safegraph_place_id,seg_int=seg_int_rw*5/8)
...
R<-map_dfr(seeds,function(s){d<-mkdat(s)
  ...
  bind_rows(ty,bi)})
saveRDS(R,"results/holc_nonlocal_hetero_FRACCORR.rds")
