# Does the DIVERSITY-benchmark relationship survive the boundary DiD?
# Same entropy-balanced CD-B design, DV = race_seg_unadj_mean (diversity benchmark, no MSA adj).
suppressMessages({library(tidyverse); library(haven); library(fixest)})
EB_SET<-"all"  # baseline: all 1910-1930 covariate gaps + missingness indicators ("key4" = original)
...
DIVF<-dplyr::as_tibble(readRDS("results/poi_nonlocal_seg_FRACCORR.rds"))%>%dplyr::transmute(safegraph_place_id,seg_div_f=seg_div)
poi<-places_usa_2019%>%inner_join(DIVF,by="safegraph_place_id")%>%mutate(seg_div=seg_div_f,lvis=log(raw_visit_counts+1),tract11=substr(poi_cbg,1,11))%>%
  filter(!is.na(seg_div),!is.na(lvis))%>%select(tract11,sub_category,seg_div,lvis)
...
est<-function(seed){side<-bt%>%group_by(boundary_id,real,boundary_side)%>%summarise(grade=first(holc_grade[holc_grade%in%c("A","B","C","D")]),across(all_of(cvars),~mean(.x,na.rm=T)),.groups="drop")%>%group_by(boundary_id)%>%filter(n()==2)%>%ungroup()
  ...
  m<-suppressMessages(feols(seg_div~lgs+lgs:real|boundary_id+sub_category,dat,weights=~w,cluster=~boundary_id))
  tibble(did=unname(coef(m)["lgs:real"]),p=unname(pvalue(m)["lgs:real"]),plac=unname(coef(m)["lgs"]),real=unname(coef(m)["lgs"]+coef(m)["lgs:real"]))}
r<-map_dfr(1:100,est)
cat("DIVERSITY-benchmark boundary DiD (entropy-balanced, 100 draws):\n")
cat(sprintf("  Real effect=%+.5f | Placebo=%+.5f | DiD=%+.5f | 95%% CI=[%+.5f,%+.5f] | median p=%.3f | %%p<.05=%.0f%%\n",
  mean(r$real),mean(r$plac),mean(r$did),quantile(r$did,.025),quantile(r$did,.975),median(r$p),100*mean(r$p<.05)))
