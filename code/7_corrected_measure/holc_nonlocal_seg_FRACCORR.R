...
# Per POI, from POST-STRATIFIED flows (visitors x CBG adjustment factor): visitor race composition
# for all / excl-own-CBG / excl-own-tract; segregation = (5/8) * sum_g |pi_g - delta_g^MSA|.
# Saves results/poi_nonlocal_seg_FRACCORR.rds. DiD is step 2.
suppressMessages({library(data.table)})
grp <- c("black","white","hispanic","asian","other")
cr <- fread("data/cbg_race_frac_2019.csv", colClasses=list(character="GEOID"))   # FRACTIONAL composition (ACS B03002)
...
# CORRECTED: post-stratification adjustment factor per home CBG (same construction as
# compute_seg_reweighted.R), applied to visitor flows before composition sums.
panel <- fread("data/home_panel_summary_2019.csv")
panel[, cbg := formatC(as.character(census_block_group), width=12, flag="0")]
panel <- panel[, .(devices = mean(number_devices_residing, na.rm=TRUE)), by=cbg]
adj <- merge(cr[,.(cbg,tot_pop)], panel, by="cbg")[devices>0 & tot_pop>0]
adj[, adjust_factor := (tot_pop/sum(tot_pop))/(devices/sum(devices))]
cr <- merge(cr, adj[,.(cbg,adjust_factor)], by="cbg")   # inner join: CBGs with panel data
setkey(cr,cbg)
...
for(f in list.files("data/flows_2019","\\.csv\\.gz$",full.names=TRUE)){
  ...
  dt[, visitors := visitors*adjust_factor]   # CORRECTED: post-stratified flows
...
divfun <- function(numer,denom){ pi <- numer/denom; rowSums(abs(pi - 0.2))*5/8 }
P[, `:=`(seg_all=segfun(Aall,t), seg_excbg=segfun(Aall-Aoc,t-toc), seg_extract=segfun(Aall-Aot,t-tot), seg_div=divfun(Aall,t),
         share_cbg=toc/t, share_tract=tot/t, tract11=substr(poi_cbg,1,11))]
P <- P[t>0 & is.finite(seg_all)]
saveRDS(P[,.(safegraph_place_id,tract11,seg_all,seg_excbg,seg_extract,seg_div,share_cbg,share_tract,t)], "results/poi_nonlocal_seg_FRACCORR.rds")
