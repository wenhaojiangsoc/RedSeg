...
OUT<-readRDS("results/holc_frac_batch_CORR.rds")
med<-OUT$mediation%>%transmute(group=mediator,c_total=c,a=a,b=b,indirect=ind,z=z,p=2*pnorm(-abs(z)),prop_mediated=prop)
saveRDS(list(med=med,dev=resdev),"results/holc_mediation_groups_CORR.rds")
saveRDS(OUT$cat7,"results/holc_category7_CORR.rds")
saveRDS(OUT$catch6%>%mutate(nb=6),"results/holc_catch_multi_CORR.rds")
