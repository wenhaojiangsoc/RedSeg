...
R1<-map_dfr(1:100,function(s){d<-if(s==1)d1 else mkdat(s)
  ...
  ps<-d%>%distinct(boundary_id,real,w_att)   # probit IPW at boundary level not re-derivable here; skip
  tibble(seed=s,plac=unname(coef(m)["lgs"]),did=unname(coef(m)["lgs:real"]),se=unname(se(m)["lgs:real"]),p=unname(pvalue(m)["lgs:real"]))})
saveRDS(list(draws=R1,sd_within=sd_tract),"results/holc_frac_tract2.rds")
...
drawp<-function(seed){set.seed(seed)
  ...
  tibble(seed=seed,plac=unname(coef(m)["lgs"]),did=unname(coef(m)["lgs:real"]),se=unname(se(m)["lgs:real"]),p=unname(pvalue(m)["lgs:real"]))}
...
saveRDS(list(draws=R2,sd_within=sd_poi),"results/holc_frac_poi2.rds")
