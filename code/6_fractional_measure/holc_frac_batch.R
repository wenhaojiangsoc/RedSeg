...
catgrp<-function(tc){tc<-as.character(tc);case_when(
  ...
  TRUE~"Other")}
fr<-as_tibble(readRDS("results/poi_ei_bounds_frac.rds"))%>%transmute(safegraph_place_id,seg_frac=seg_eco*5/8)
...
cat("=== (a) RAW boundary discontinuity, real borders only (Col 2 analog) ===\n")
dr<-dats[[1]]%>%filter(real==1)
m2<-feols(seg_frac~lgs,dr,fixef=c("boundary_id","sub_category"),weights=~lvis,cluster=~boundary_id)
OUT$col2<-c(b=unname(coef(m2)["lgs"]),se=unname(se(m2)["lgs"]),p=unname(pvalue(m2)["lgs"]))
cat(sprintf("raw disc %+.5f (se %.5f, p=%.4f)\n",OUT$col2["b"],OUT$col2["se"],OUT$col2["p"]))
cat("\n=== (b) MEDIATION, overall + by group (seeds 1/3/13) ===\n")
...
mediate<-function(dat,med){dat$M<-dat[[med]]
  mc<-feols(seg_frac~lgs+lgs:real|boundary_id+sub_category,dat,weights=~w,cluster=~boundary_id)
  ma<-feols(M~lgs+lgs:real|boundary_id+sub_category,dat,weights=~w,cluster=~boundary_id)
  mb<-feols(seg_frac~lgs+lgs:real+M|boundary_id+sub_category,dat,weights=~w,cluster=~boundary_id)
  cc<-cf(mc,"lgs:real");a<-cf(ma,"lgs:real");b<-cf(mb,"M")
  ind<-a["b"]*b["b"];se_ind<-sqrt(b["b"]^2*a["s"]^2+a["b"]^2*b["s"]^2)
  c(c=unname(cc["b"]),a=unname(a["b"]),a_se=unname(a["s"]),b=unname(b["b"]),b_se=unname(b["s"]),ind=unname(ind),z=unname(ind/se_ind),prop=unname(ind/cc["b"]))}
meds<-c(overall="absdev_total",black="absdev_black",white="absdev_white",hispanic="absdev_hispanic",asian="absdev_asian")
med_res<-map_dfr(names(meds),function(g){M<-sapply(dats,function(d)mediate(d,meds[[g]]));r<-rowMeans(M)
  tibble(mediator=g,!!!as.list(r))})
OUT$mediation<-med_res; OUT$direction<-dir
print(as.data.frame(med_res%>%mutate(across(-mediator,~round(.x,5)))),row.names=FALSE)
cat("\n=== (c) AMENITY-MIX decomposition (seeds 1/3/13) ===\n")
am<-map_dfr(dats,function(d){
  mt<-feols(seg_frac~lgs+lgs:real,d,fixef=c("boundary_id"),weights=~w,cluster=~boundary_id)
  mm<-feols(m_cat~lgs+lgs:real,d,fixef=c("boundary_id"),weights=~w,cluster=~boundary_id)
  mr<-feols(res_cat~lgs+lgs:real,d,fixef=c("boundary_id"),weights=~w,cluster=~boundary_id)
  tibble(tot=coef(mt)["lgs:real"],amen=coef(mm)["lgs:real"],amen_p=pvalue(mm)["lgs:real"],within=coef(mr)["lgs:real"])})
OUT$amenity<-am%>%summarise(across(everything(),mean))
with(OUT$amenity,cat(sprintf("total %+.5f | amenity-mix %+.5f (p=%.2f) -> %.0f%% | within %+.5f\n",tot,amen,amen_p,100*amen/tot,within)))
cat("\n=== (d) CATEGORY heterogeneity, 7 types (seeds 1/3/13) ===\n")
keep<-c("Religious","Retail & grocery","Food & drink","Personal care","Recreation & culture","Education & childcare","Health care")
cg<-map_dfr(seq_along(seeds),function(i){d<-dats[[i]]
  map_dfr(keep,function(g){dg<-d%>%filter(catgroup==g);if(n_distinct(dg$lgs)<2||nrow(dg)<200)return(NULL)
    m<-tryCatch(feols(seg_frac~lgs+lgs:real|boundary_id+sub_category,dg,weights=~w,cluster=~boundary_id),error=function(e)NULL)
    if(is.null(m))return(NULL);tibble(seed=seeds[i],catgroup=g,est=coef(m)["lgs:real"],se=se(m)["lgs:real"],n=nrow(dg))})})
OUT$cat7<-cg
cg%>%group_by(catgroup)%>%summarise(est=mean(est),t=mean(est)/mean(se),.groups="drop")%>%arrange(desc(est))%>%
  mutate(est=round(est,5),t=round(t,2),p=round(2*pnorm(-abs(t)),3))%>%as.data.frame()%>%print(row.names=FALSE)
cat("\n=== (e) CATCHMENT heterogeneity, 6 bins (seeds 1/3/13) ===\n")
qn<-map_dfr(seq_along(seeds),function(i){d<-dats[[i]]%>%filter(!is.na(catch))%>%mutate(b=ntile(catch,6))
  map_dfr(1:6,function(k){dk<-d%>%filter(b==k);m<-feols(seg_frac~lgs+lgs:real|boundary_id+sub_category,dk,weights=~w,cluster=~boundary_id)
    tibble(seed=seeds[i],bin=k,est=coef(m)["lgs:real"],se=se(m)["lgs:real"],meancatch=mean(dk$catch,na.rm=TRUE))})})
OUT$catch6<-qn
qn%>%group_by(bin)%>%summarise(est=mean(est),t=mean(est)/mean(se),km=round(mean(meancatch)/1000,1),.groups="drop")%>%
  mutate(est=round(est,5),t=round(t,2))%>%as.data.frame()%>%print(row.names=FALSE)
rm(dats);gc(FALSE)
...
R<-map_dfr(1:100,function(s){d<-mkdat(s,ipw=TRUE)
  ...
  tibble(seed=s,did=unname(coef(m)["lgs:real"]),se=unname(se(m)["lgs:real"]),p=unname(pvalue(m)["lgs:real"]))})
OUT$ipw<-R
cat(sprintf("IPW DiD %+.5f | rand CI [%+.5f,%+.5f] | %%p<.05 %.0f\n",mean(R$did),quantile(R$did,.025),quantile(R$did,.975),100*mean(R$p<.05)))
saveRDS(OUT,"results/holc_frac_batch.rds")
cat("Saved results/holc_frac_batch.rds\n")
