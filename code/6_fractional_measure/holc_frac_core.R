...
saveRDS(list(draws=R1,sd_within=sd_tract),"results/holc_frac_tract.rds")
...
drawp<-function(seed){set.seed(seed)
  ...
  tibble(seed=seed,did=unname(coef(m)["lgs:real"]),se=unname(se(m)["lgs:real"]),p=unname(pvalue(m)["lgs:real"]))}
...
saveRDS(list(draws=R2,sd_within=sd_poi),"results/holc_frac_poi.rds")
