# FRACTIONAL campaign, part 3: write the Figure 4/5 input rds files (canonical formats) from
# the fractional batch results, adding the Panel-B signed-displacement regressions that the
# batch did not compute, then regenerate both figures. Majority-based inputs preserved as
# results/*_MAJORITY.rds.
...
for(g in names(grps)){nb<-grps[[g]];ms<-paste0("msa_",nb)
  poi[[paste0("dev_",g)]]<-poi[[nb]]-poi[[ms]]}
poi<-poi%>%filter(!is.na(seg_frac),!is.na(lvis),!is.na(sub_category))%>%
  select(tract11,sub_category,seg_frac,lvis,starts_with("dev_"))
...
cat("== signed displacement (Panel B), fractional ==\n")
resdev%>%mutate(t=est/se,p=2*pnorm(-abs(t)),across(c(est,se),~round(.x,5)))%>%as.data.frame()%>%print(row.names=FALSE)
OUT<-readRDS("results/holc_frac_batch.rds")
med<-OUT$mediation%>%transmute(group=mediator,c_total=c,a=a,b=b,indirect=ind,z=z,p=2*pnorm(-abs(z)),prop_mediated=prop)
saveRDS(list(med=med,dev=resdev),"results/holc_mediation_groups.rds")
saveRDS(OUT$cat7,"results/holc_category7.rds")
saveRDS(OUT$catch6%>%mutate(nb=6),"results/holc_catch_multi.rds")
cat("Wrote fractional fig inputs to canonical rds names (majority backed up as *_MAJORITY.rds)\n")
