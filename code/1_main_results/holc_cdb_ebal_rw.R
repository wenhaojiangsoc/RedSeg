# CD-B boundary DiD (entropy balancing, EB_SET="all") on the SafeGraph-REWEIGHTED integration
# segregation measure (seg_int_rw from compute_seg_reweighted.R / poi_seg_reweighted.rds).
# Identical design to holc_cdb_ebal.R; the only change is the outcome: seg_int = seg_int_rw*5/8
# in place of race_seg_msa_adj_mean*5/8. Robustness check for Reviewer 1's post-stratification ask.
...
ebalance<-function(X,target,maxit=300,tol=1e-8){
  Z<-sweep(X,2,target,"-"); n<-nrow(Z); base<-rep(1/n,n); lam<-rep(0,ncol(Z)); w<-base
  for(it in 1:maxit){u<-as.vector(Z%*%lam);u<-u-max(u);w<-base*exp(u);w<-w/sum(w)
    g<-colSums(w*Z); if(max(abs(g))<tol)break
    H<-t(Z)%*%(w*Z)-outer(g,g); step<-tryCatch(solve(H+diag(1e-8,ncol(Z)),g),error=function(e)g)
    best<-max(abs(g)); lam0<-lam
    for(s in c(1,.5,.25,.1,.03)){lt<-lam0-s*step;ut<-as.vector(Z%*%lt);ut<-ut-max(ut);wt<-base*exp(ut);wt<-wt/sum(wt)
      gt<-colSums(wt*Z);if(max(abs(gt))<best){lam<-lt;best<-max(abs(gt));break}}
    if(identical(lam,lam0))break}
  w}
estimate<-function(seed,yv){
  ...
  wp<-ebalance(Xp,tgt)*sum(gs$real==0)
...
cat("Entropy-balanced CD-B difference-in-discontinuity on RECOMPUTED measures, 100 draws.\n")
...
saveRDS(r_unw,"results/holc_cdb_ebal_unw_draws.rds"); saveRDS(r_rw,"results/holc_cdb_ebal_rw_draws.rds")
