...
pub<-places_usa_2019%>%transmute(safegraph_place_id,seg_pub=race_seg_msa_adj_mean*5/8)
real<-readRDS("data/poi_real_assign.rds")%>%left_join(pub,by="safegraph_place_id")%>%
  transmute(safegraph_place_id,boundary_id=paste0("R",boundary_id),real=1L,lgs=as.integer(side=="low"),
            gside=NA_real_,grade,tract11,sub_category,lvis,seg_pub)
...
plac<-plac%>%left_join(pub,by="safegraph_place_id")%>%filter(boundary_id%in%valid)%>%
  transmute(safegraph_place_id,boundary_id,real=0L,lgs=NA_integer_,gside,grade,tract11,sub_category,lvis,seg_pub)
...
draw<-function(seed){
  ...
  fit("seg_pub","w_eb")%>%mutate(scheme="EB",measure="Published",seed=seed,eb_bal=eb_bal)}

cat("running 100 random-orientation draws (POI-level DiD)...\n")
R<-map_dfr(1:100,draw)
saveRDS(R,"results/holc_col5_poi_pub_draws.rds")
