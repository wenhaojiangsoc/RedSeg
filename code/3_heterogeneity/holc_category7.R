...
cg<-map_dfr(seeds,function(s){d<-mkdat(s)
  map_dfr(keep,function(g){dg<-d%>%filter(catgroup==g);if(n_distinct(dg$lgs)<2||nrow(dg)<200)return(NULL)
    m<-tryCatch(feols(seg_int~lgs+lgs:real|boundary_id+sub_category,dg,weights=~w,cluster=~boundary_id),error=function(e)NULL)
    if(is.null(m))return(NULL);tibble(seed=s,catgroup=g,est=coef(m)["lgs:real"],se=se(m)["lgs:real"],n=nrow(dg))})})
saveRDS(cg,"results/holc_category7.rds")
cat("=== 7 clean categories (auto/fuel excluded) ===\n")
cg%>%group_by(catgroup)%>%summarise(est=mean(est),t=mean(est)/mean(se),n=round(mean(n)),.groups="drop")%>%arrange(desc(est))%>%
