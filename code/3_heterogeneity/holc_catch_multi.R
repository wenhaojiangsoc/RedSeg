# Heterogeneity of the EB CD-B boundary DiD (reweighted measure):
#   (1) Catchment interaction -- does the redlining effect vary with how far a POI's visitors travel?
#       (distance_from_home, the SafeGraph visitor-travel distance = catchment size). Interaction +
#       tercile-split treatment effects.
#   (2) Category heterogeneity -- treatment effect estimated within ~7 broad establishment types.
# Same EB machinery as holc_cdb_mediation_rw.R; effects averaged over 3 placebo-orientation seeds.
...
catgrp<-function(tc){tc<-as.character(tc);case_when(
  grepl("Restaurant|Eating|Drinking|Bar|Beverage|Snack",tc)~"Food & drink",
  grepl("Religious",tc)~"Religious",
  grepl("Physician|Dentist|Health Practitioner|Medical|Hospital|Nursing|Outpatient|Mental",tc)~"Health care",
  grepl("Grocery|Personal Care Stores|Gasoline|Clothing|Department|Supermarket|Convenience|Pharmac|Drug|Merchandise|Book|Hardware|Florist|Shoe",tc)~"Retail & grocery",
  grepl("School|Day Care|Colleges|Universit|Educational|Instruction",tc)~"Education & childcare",
  grepl("Amusement|Recreation|Museum|Historical|Fitness|Golf|Bowling|Arts|Sports|Gambling|Zoo|Park",tc)~"Recreation & culture",
  grepl("Personal Care Services|Automotive Repair|Beauty|Barber|Laundry|Repair|Maintenance",tc)~"Personal & auto services",
  TRUE~"Other")}
poi<-places_usa_2019%>%
  ...
  mutate(seg_int=seg_int_rw*5/8,lvis=log(raw_visit_counts+1),tract11=substr(poi_cbg,1,11),
         catch=distance_from_home,lcatch=log(distance_from_home+1),catgroup=catgrp(top_category))%>%
  filter(!is.na(seg_int),!is.na(lvis))%>%
  select(tract11,sub_category,catgroup,seg_int,lvis,catch,lcatch)
...
out<-list()
for(NB in c(6,7)){
  qn<-purrr::map_dfr(seeds,function(s){d<-mkdat(s)%>%filter(!is.na(catch));d<-d%>%mutate(b=ntile(catch,NB))
    purrr::map_dfr(1:NB,function(k){dk<-d%>%filter(b==k);m<-feols(seg_int~lgs+lgs:real|boundary_id+sub_category,dk,weights=~w,cluster=~boundary_id)
      tibble(nb=NB,seed=s,bin=k,est=coef(m)["lgs:real"],se=se(m)["lgs:real"],meancatch=mean(dk$catch,na.rm=TRUE))})})
  out[[as.character(NB)]]<-qn
  cat(sprintf("\n=== %d bins ===\n",NB))
  qn%>%group_by(bin)%>%summarise(est=mean(est),t=mean(est)/mean(se),mc=round(mean(meancatch)),.groups="drop")%>%mutate(est=round(est,5),t=round(t,2))%>%as.data.frame()%>%print(row.names=FALSE)
}
saveRDS(bind_rows(out),"results/holc_catch_multi.rds")
