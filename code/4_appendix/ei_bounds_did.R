# ECOLOGICAL-INFERENCE robustness (step 2 of 2): entropy-balanced boundary DiD on the recomputed segregation
# variants from ei_bounds_seg.R -- seg_eco (baseline, should reproduce the ~+0.0055 main effect), seg_lower /
# seg_upper (Duncan-Davis method-of-bounds), and seg_h07 / seg_h09 (homogeneity-restricted, tau=0.7/0.9).
# Same design/weights/clustering/orientations as the main estimator (holc_nonlocal_did.R).
...
S <- as_tibble(readRDS("results/poi_ei_bounds.rds")) %>% rename(seg_h07=`seg_h0.7`, seg_h09=`seg_h0.9`)
...
poi<-S%>%left_join(meta,by="safegraph_place_id")%>%mutate(lvis=log(t+1))%>%
  ...
  select(tract11,sub_category,seg_eco,seg_lower,seg_upper,seg_h07,seg_h09,lvis)
...
outs<-c("seg_eco","seg_lower","seg_upper","seg_h07","seg_h09")
...
agg<-aggregate(cbind(b,s,n)~outcome, R, mean)
agg<-agg[match(outs,agg$outcome),]
cat("\n=== ECOLOGICAL-INFERENCE robustness: boundary DiD on seg variants (avg 5 orientations) ===\n")
lab<-c(seg_eco="baseline (ecological, main)", seg_lower="Duncan-Davis LOWER bound", seg_upper="Duncan-Davis UPPER bound",
       seg_h07="homogeneity tau>=0.70", seg_h09="homogeneity tau>=0.90")
for(i in seq_len(nrow(agg))){ r<-agg[i,]
  cat(sprintf("  %-30s beta= %+.5f (se %.5f, t=%.2f, p=%.4f, n=%d)\n", lab[r$outcome], r$b, r$s, r$b/r$s, 2*pnorm(-abs(r$b/r$s)), round(r$n))) }
saveRDS(agg, "results/ei_bounds_did.rds")
