...
real<-readRDS("data/poi_real_assign.rds")%>%transmute(safegraph_place_id,boundary_id=paste0("R",boundary_id),real=1L,lgs=as.integer(side=="low"),
            gside=NA_real_,grade,tract11,sub_category,lvis,seg_rw=seg_int_rw*5/8)
...
plac<-plac%>%filter(boundary_id%in%valid)%>%transmute(safegraph_place_id,boundary_id,real=0L,lgs=NA_integer_,gside,grade,tract11,sub_category,lvis,seg_rw=seg_int_rw*5/8)
...
saveRDS(list(est=R,bal=B,sweep=SW),"results/holc_lowprop_poi.rds")
