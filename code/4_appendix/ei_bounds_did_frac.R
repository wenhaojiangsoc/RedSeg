...
S <- as_tibble(readRDS("results/poi_ei_bounds_frac.rds"))
...
poi<-S%>%left_join(meta,by="safegraph_place_id")%>%mutate(lvis=log(t+1))%>%
  ...
  select(tract11,sub_category,seg_eco,seg_lower,seg_upper,seg_h08,lvis)
...
outs<-c("seg_eco","seg_lower","seg_upper","seg_h08")
...
saveRDS(agg, "results/ei_bounds_did_frac.rds")
